  ...
  const handleGenerateJoke = async () => {
    ...
    try {
      const result = await createEvent('chatgpt_request', {
        prompt:
          'Give me a joke in JSON format with the following structure: { "setup": "joke setup", "punchline": "joke punchline" }',
        response_type: 'json',
      });
      setNewJoke(result);
    } catch (error) {
      ...
      >
        <div class="max-w-6xl mx-auto h-full flex flex-col">
          <div class="flex justify-between items-center mb-8">
            <h1 class="text-4xl font-bold text-purple-600">Jokes app</h1>
            <button
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 flex-grow">
            <div class="col-span-1 md:col-span-2 lg:col-span-1">
              ...
              <form onSubmit={saveJoke} class="space-y-4">
                ...
                <div class="flex space-x-4">
                  <button
                    type="submit"
                    class={`flex-1 px-6 py-3 bg-purple-500 text-white rounded-lg transition duration-300 ease-in-out transform hover:scale-105 cursor-pointer ${
                  ...
                  <button
                    type="button"
                    class={`flex-1 px-6 py-3 bg-blue-500 text-white rounded-lg transition duration-300 ease-in-out transform hover:scale-105 cursor-pointer ${
            ...
            <div class="col-span-1 md:col-span-2 lg:col-span-1">
              ...
              <div class="space-y-4 max-h-[calc(100vh-300px)] overflow-y-auto pr-4">
                <Show when={!loading()} fallback={<p>Loading jokes...</p>}>
                  <For each={jokes()}>
                    {(joke) => (
                      <div class="bg-white p-6 rounded-lg shadow-md transition duration-300 ease-in-out transform hover:scale-105 flex justify-between items-start">
                        <div>
            ...
            <div class="col-span-1 md:col-span-2 lg:col-span-1">
              ...
              <div class="space-y-4">
                <button
                  ...
                  class={`w-full px-6 py-3 bg-green-500 text-white rounded-lg transition duration-300 ease-in-out transform hover:scale-105 cursor-pointer ${
                    loading() ? 'opacity-50 cursor-not-allowed' : 'hover:bg-green-600'
                  }`}
                  disabled={loading()}
                >
                  <Show when={!loading()} fallback={<span>Generating Image...</span>}>
                    Generate Image
                  </Show>
                </button>
                <Show when={newJoke().setup && newJoke().punchline}>
                  <button
                    ...
                    class={`w-full px-6 py-3 bg-yellow-500 text-white rounded-lg transition duration-300 ease-in-out transform hover:scale-105 cursor-pointer ${
                      loading() ? 'opacity-50 cursor-not-allowed' : 'hover:bg-yellow-600'
                    }`}
                    disabled={loading()}
                  >
                    <Show when={!loading()} fallback={<span>Converting to Speech...</span>}>
                      Text to Speech
                    </Show>
                  </button>
                </Show>
                <button
                  ...
                  class={`w-full px-6 py-3 bg-indigo-500 text-white rounded-lg transition duration-300 ease-in-out transform hover:scale-105 cursor-pointer ${
                    loading() ? 'opacity-50 cursor-not-allowed' : 'hover:bg-indigo-600'
                  }`}
                  disabled={loading()}
                >
                  <Show when={!loading()} fallback={<span>Generating Story...</span>}>
                    Generate Markdown
                  </Show>
                </button>
              </div>
